		...
		<!-- Content Row -->
			<div class="row">
				<div class="col-lg-4 mb-4">
					<div class="card h-60 bg-03">
						<div class="card-header">
							<div class="display-4"><b>Enter Airline_ID</b></div>
							<div class="font-italic"><br><br><br><br></div>
                        		<form name="sentMessage" id="contactForm" novalidate>
                                    <div class="control-group form-group">
                        				<div class="controls">
                           					<input type="text" placeholder="Enter Airline_ID" class="form-control" id="name" name='id' required data-validation-required-message="Please enter Airline_ID.">
                                    ...
                                    <center><button type="submit" class="btn btn-primary" id="sendMessageButton">GO</button></center>
                         		</form>
							<?php
						...
						<?php
                            $sql = "SELECT airlineid,flight_code,source,destination,duration,arrival,departure FROM flight where airlineid='".$id."' ";
				            $result =$connection-> query($sql) ;
                            if (isset($_GET['id'])){
                                if(mysqli_num_rows($result) > 0){
                                    while($row = $result->fetch_assoc()) {
                						echo "<li class='list-group-item'>Al_Code:- ".$row['flight_code']."<br>".$row['departure']." - ".$row['arrival']." <br>".$row['source']." To ".$row['destination']."<br>Duration:- ".$row['duration']."</li>";
                                    }
                                }
                                else{
				...
				<div class="col-lg-4 mb-4">
					<div class="card h-60 bg-01">
						<div class="card-header">
							<h3 class="">Airlines ID</h3>
						</div>
						<ol class="list-group list-group-flush">
						<?php
                            $sql2 = "SELECT al_name,airlineid FROM airline";
				            $result2 =$connection-> query($sql2) or die ("DBMS not connected");
                                if (isset($_GET['id'])){
                                    if (mysqli_num_rows($result2) > 0) {
                                        while($row = $result2->fetch_assoc()) {
                                            echo "<li class='list-group-item'>".$row['al_name']." -> ".$row['airlineid']."</li>";
                                        }
                                    }
                                }
                                else {
