...
<div class="wrapper-main">
	...
	<div class="full-title">
		<div class="container">
			...
			<h1 class="mt-4 mb-3">Employee</h1>
    ...
    <div class="container">
		...
		<form name="sentMessage" id="contactForm" novalidate>
            <div class="control-group form-group">
				<div class="controls">
   					<input type="text" placeholder="Enter SSN Number Of Employee" class="form-control" id="name" name='SSN' required data-validation-required-message="Please enter employee SSN number.">
			...
			<div class="row">
					...
					<div class="card h-100">
						...
						<div class="card-body">
							...
							<p class="card-text">
							    <?php
							        settype($ssn,'integer');
							        $sql1 = "SELECT fname,m,lname FROM employee1 where ssn=$ssn";
				                    $result =$connection-> query($sql1) or die ("DBMS not connected");
                                    if (isset($_GET['SSN'])){
					...
					<div class="card h-100">
						...
						<div class="card-body">
							...
							<p class="card-text">
	                            <?php
    							    settype($ssn,'integer');
    							    $sql2 = "SELECT address FROM employee1 where ssn=$ssn";
    				                $result =$connection-> query($sql2) or die ("DBMS not connected");
                                    if (isset($_GET['SSN']) ){
                                        if (mysqli_num_rows($result) > 0) {
                                            while($row = $result->fetch_assoc()) {
					...
					<div class="card h-100">
						...
						<div class="card-body">
							...
							<p class="card-text">
                                <?php
    							    settype($ssn,'integer');
    							    $sql3 = "SELECT phone FROM employee1 where ssn=$ssn";
    				                $result =$connection-> query($sql3) or die ("DBMS not connected");
                                    if (isset($_GET['SSN'])){
                                        if (mysqli_num_rows($result) > 0) {
                                            while($row = $result->fetch_assoc()) {
			   ...
			   <div class="col-lg-4 mb-4">
					<div class="card h-100">
						...
						<div class="card-body">
							<h4 class="card-header"> Age,Sex </h4>
							<p class="card-text">
                                <?php
    							    settype($ssn,'integer');
    							    $sql4 = "SELECT age,sex FROM employee1 where ssn=$ssn";
    				                $result =$connection-> query($sql4) or die ("DBMS not connected");
                                    if (isset($_GET['SSN']) ){
                                        if (mysqli_num_rows($result) > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                if ($row['sex']=='F'){
                                                    echo "<h5>". $row['age']. " FEMALE</h5>";
                                                }
                                                else{
                                                    echo "<h5>". $row['age']. " MALE</h5>";
                                                }
                                            }
                                        }
                                    }
			   ...
			   <div class="col-lg-4 mb-4">
					<div class="card h-100">
						<div class="card-img">
							<img class="img-fluid" src="salary.png" alt="" />
						</div>
						<div class="card-body">
							<h4 class="card-header"> JobType[salary] </h4>
							<p class="card-text">
                                <?php
    							    settype($ssn,'integer');
    							    $sql5 = "SELECT employee1.jobtype,employee2.salary FROM employee1 JOIN employee2 ON employee1.jobtype=employee2.jobtype where ssn=$ssn ";
    				                $result =$connection-> query($sql5) or die ("DBMS not connected");
                                    if (isset($_GET['SSN']) ){
                                        if (mysqli_num_rows($result) > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                echo "<h5>". $row['jobtype']. "[".$row['salary']."]</h5>";
                                            }
                                        }
                                        else{
                                            $sql_bu="SELECT jobtype FROM employee1 where ssn=$ssn";
                                            $result_bu=$connection-> query($sql_bu) or die ("DBMS not connected");
                                            while($row_bu = $result_bu->fetch_assoc()) {
                                                echo "<h5>". $row_bu['jobtype']. "<b><br> SALARY NOT MENTIONED</b></h5>";
                                            }
                                        }
                                    }
                                    else {
			   ...
			   <div class="col-lg-4 mb-4">
					<div class="card h-100">
						...
						<div class="card-body">
							<h4 class="card-header"> Assi. Type </h4>
							<p class="card-text">
                                <?php
    							    settype($ssn,'integer');
    							    $sql6 = "SELECT astype FROM employee1 where ssn=$ssn ";
    				                $result =$connection-> query($sql6) or die ("DBMS not connected");
                                    if (isset($_GET['SSN'])){
                                        if (mysqli_num_rows($result) > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                if ($row['astype']!=NULL){
                                                    echo "<h5>". $row['astype']. "</h5>";
                                                }
                                                else{
                                                    echo "<h5>NOT MENTIONED.</h5>";
                                                }
                                            }
                                        }
                                    }
    ...
    <div class="container">
    		...
    		<form name="sentMessage" id="contactForm" novalidate>
                <div class="control-group form-group">
    				<div class="controls">
                        <center><a class='btn btn-lg btn-secondary btn-button' href='services.php?SSN=0'>Clear Data</a></center>
    				</div>
    			</div>
    		</form>
    </div>
